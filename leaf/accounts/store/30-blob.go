...
// Per-handle usage accounting is stored as ONE row per handle with a single JSON
// blob (bob_account_handle_usage.usage_json) — a map of period-key → counts,
// downsampled by age so the blob stays bounded forever while the summed total
// stays exact (true lifetime):
//
//	< 3 months : daily   key "2006-01-02"
//	3mo – 3yr  : monthly key "2006-01"
//	> 3 years  : yearly  key "2006"
//
// ~90 daily + ~33 monthly + ~100 yearly ≈ 220 keys per century (a few KB). All
// the logic is pure Go here; the store just SELECTs the blob and UPSERTs the
// result. A turn is recorded in ONE read-modify-write (BumpTurnUsage): turns +
// successes, per-KIND token counts (K — llm/ocr/translate, each {in,out}), and
// per-service NATIVE-unit counts (V — "kind:unit"→amount). Tokens (K) and native
// units (V) are structurally disjoint so a sum of one never folds in the other.
...
type usageBucket struct {
	...
	// I/O are pre-per-kind aggregate token counts. LEGACY: buckets written before
	// the per-kind split carry them; new writes use K instead. Still summed into
	// the grand total so old data isn't lost.
	...
	// V is per-service NATIVE-unit usage, keyed "kind:unit" → amount. Disjoint
...
// fresh; a malformed blob is reset fail-open (WARN + fresh map) so this write
// overwrites the corrupt data and the handle self-heals. One marshal per turn —
// the single accounting write.
func BumpTurnUsage(blob string, turns, success int64, tokens map[string]KindTokens, native map[string]int64, now time.Time) (string, error) {
	...
	if err != nil {
		// Fail OPEN (matches the read seams, which treat a malformed blob as zero): reset
		// to an empty map so THIS write overwrites the corrupt blob and the handle
		// self-heals, instead of latching the error forever (the buffer would otherwise
		// re-stage this entry every flush for the life of the process). The corrupt buckets
		// are already unreadable, so nothing recoverable is lost.
		slog.Warn("accounts: malformed usage_json reset (fail-open self-heal)", "err", err)
...
// three read seams. The store's per-account JOIN calls this once per bound
// handle. Lazy-inits the maps.
...
// compactUsage folds daily buckets older than 3 months into their month key,
// then monthly buckets older than 3 years into their year key. Precision is told
// from the key length (10=day, 7=month, 4=year). Lossless (sums carry forward).
